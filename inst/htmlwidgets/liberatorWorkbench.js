(function () {
  ...
  function AssessmentModal(props) {
    var mode=props.mode||"static",scope=React.useState("automatic"),count=React.useState("2"),since=React.useState(""),info=props.owner.profileObservation||{},readiness=props.owner.readiness||{},dynamic=mode==="dynamic";
    ...
    return e(Modal,{title:dynamic?"Estimate parameters changing over time":"Estimate stable patient parameters",subtitle:dynamic?"Fit separate patient states across declared boundaries":"Fit one patient-specific parameter state across the available evidence",onClose:props.onClose},
      ...
      e("div",{className:"lr-callout lr-callout-info"},e("strong",null,"Evidence used for estimation"),e("p",null,"All eligible patient evidence through the current timeline cutoff remains available to the posterior fit. The option below controls which TDM observations are displayed over the individualised dosing-interval curve.")),
      e("div",{className:"lr-form-grid"},
      ...
      e("footer",{className:"lr-modal-actions"},e(Button,{onClick:props.onClose},"Cancel"),e(Button,{className:"lr-primary",disabled:!valid||(dynamic&&!readiness.dynamicReady),onClick:function(){emit(props.owner,"assess",{mode:mode,process_scale:.1,profile_observation_scope:scope[0],profile_observation_count:Number(count[0]),profile_observation_since:scope[0]==="since"?Number(since[0]):null});props.onClose();}},dynamic?"Run time-changing assessment":"Run stable assessment")));
  }
  ...
  function EndpointSetModal(props) {
    ...
    return e(Modal,{className:"lr-modal-wide",title:edit?"Modify multi-endpoint objective":"Combine therapeutic endpoints",subtitle:"Define one explicit primary objective, optional secondary objectives, and posterior safety constraints",onClose:props.onClose},
      ...
      e("div",{className:"lr-form-grid"},
        ...
        e(Field,{label:"Governance status"},e("select",{value:status[0],onChange:function(x){status[1](x.target.value);}},["draft","reviewed","qualified"].map(function(item){return e("option",{key:item,value:item},item);}))),
  ...
  function ParameterTable(props) {
    ...
    if(!rows.length&&!outputs.length)return e(Empty,{title:"No individual posterior",detail:"Run a static or dynamic assessment after adding TDM observations."});
  ...
  function RegimenTable(props) {
    ...
    return e("div",null,
      ...
      e("div",{className:"lr-table-wrap"},e("table",{className:"lr-table lr-regimen-table"},
        e("thead",null,e("tr",null,headers.map(function(x){return e("th",{key:x,title:x==="Joint attainment"?"Probability that all configured endpoints are met in the same posterior draw":x==="Expected utility"?"Weighted mean of component utilities after explicit normalization":x==="Hard constraints"?"Every configured posterior chance constraint must pass before selection":x==="Pareto"?"Not dominated by another eligible regimen across component expected utilities":x==="SS target attainment"?"Probability that the endpoint metric at periodic steady state meets its target":x==="SS mean"?"Posterior median time-weighted concentration over one steady-state dosing interval":x==="SS trough–peak"?"Posterior median trough and peak during one periodic steady-state interval":x==="Burden-adjusted score"?"Lower is preferred: steady-state endpoint loss plus any configured daily-dose burden":undefined},x);}))),
  ...
  function EndpointOutcomeTable(props) {
    ...
    function intervalLabel(row){var lower=number(row.lower_probability),upper=number(row.upper_probability);return lower!==null&&upper!==null?fmt(100*(upper-lower),0)+"% posterior interval":"Posterior interval";}
    function outcomeUnit(row){return String(value(row.display_unit,"")).trim();}
    return e("section",{className:"lr-endpoint-outcomes"},
      e("div",{className:"lr-endpoint-outcome-heading"},e("h4",null,rows.length>1?"Endpoint outcomes":"Endpoint outcome"),e("p",null,"Numerical posterior outcomes and the targets used for this regimen decision.")),
      e("div",{className:"lr-table-wrap"},e("table",{className:"lr-table lr-endpoint-outcome-table"},
        ...
        e("tbody",null,rows.map(function(row,index){var suffix=outcomeUnit(row),hard=!!row.hard_constraint,pass=row.constraint_pass!==false,attainment=number(row.attainment_probability),minimum=number(row.minimum_attainment);return e("tr",{key:value(row.component_id,index)},
          ...
          e("td",null,hard?e("span",{className:"lr-endpoint-rule"},e(Badge,{tone:pass?"covariate":"warning"},pass?"Pass":"Fail"),e("small",null,minimum===null?"Posterior threshold unavailable":"\u2265 "+fmt(100*minimum,0)+"% required")):e("span",{className:"lr-endpoint-rule"},e(Badge,{tone:"neutral"},"No hard gate"),e("small",null,"Contributes to weighted utility"))));})))),
      e("p",{className:"lr-profile-context lr-endpoint-outcome-note"},"Endpoint intervals are quantiles of the endpoint metric across the same individual posterior draws used for target attainment. They are not confidence intervals for a population mean."));
  }

  function ForecastOne(props) {
    var prediction=props.prediction,rows=list(prediction&&prediction.forecast);
    if(!rows.length)return null;
    var probabilities=list(prediction.probabilities),lowerProbability=number(probabilities[0]),upperProbability=number(probabilities[2]),coverage=lowerProbability!==null&&upperProbability!==null?100*(upperProbability-lowerProbability):90,intervalLabel=fmt(coverage,0)+"% posterior prediction interval (individual ETA uncertainty)";
    ...
    return e("div",{className:"lr-forecast"},
      ...
      e("div",{className:"lr-legend"},e("span",null,e("i",{className:"lr-forecast-line-key"}),"Posterior median"),e("span",null,e("i",{className:"lr-forecast-limit-key"}),"Dashed limits: "+intervalLabel),target?e("span",null,e("i",{className:"lr-target-key"}),"Therapeutic range"):null,e("span",null,e("i",{className:"lr-triangle"}),"Future dose")),
      e("p",{className:"lr-profile-context lr-forecast-interval-note"},"The dashed interval is formed from pointwise posterior quantiles for the individual's latent concentration across ETA draws. It is not a confidence interval for the mean; residual measurement variability is excluded."),
  ...
  function Forecast(props) {
    ...
    if(predicting)return e(TaskProgress,{title:"Generating future predictions",detail:"Propagating posterior uncertainty for the selected regimen(s). Forecasts will appear here when ready.",cancellable:task.cancellable,cancel:function(){emit(props,"cancel_task",{id:task.id});}});
  ...
  function RightRail(props) {
    ...
    return e("aside",{className:"lr-right"+(props.drawerOpen?" open":"")},
      e(Panel,{title:"Assessment readiness",subtitle:ready?"Ready to individualise":missing?"Next: "+missing[1]:"Review inputs"},
        ...
        readiness.anyChanged?e("p",{className:"lr-readiness-note"},"* Changed since the most recent individualisation; rerun the assessment to update the posterior."):null,
